...
def get_counts(labels):
    values, counts = np.unique(labels, return_counts=True) # two list
    # tuples_list = [(2, 3), (0, 1), (1, 2), (3, 4)]
    # sorted = [(0, 1), (1, 2), (2, 3), (3, 4)]
    # sorted_tuples = zip(0, 1, 2, 3), zip(1, 2, 3, 4)
...
class CombinedDataset(torch.utils.data.Dataset):
    def __init__(self, datasets):
        ...
        self.samples = np.concatenate([d.samples for d in datasets]) #形成一个新的NumPy数组
        ...
        self.classes = datasets[0].classes # Black footed Albatross
        self.group_names = np.concatenate([d.group_names for d in datasets])
        self.class_names = datasets[0].class_names # 001.Black_footed_Albatross
        print(f"Combining datasets of size {[len(d) for d in datasets]} \t Total = {len(self.samples)}")
        if hasattr(datasets[0], 'seg'):
            self.seg = np.concatenate([d.seg for d in datasets])
...
class BasicDataset(torchvision.datasets.ImageFolder):
    ...
    def __init__(self, root, transform=None, group=0, cfg=None):
        ...
        if not cfg or not cfg.data.extra_classes: # default; satisfied when cfg is none or when extra_classes is False
            self.class_names = self.classes
            self.class_map = None
        else: # when extra_classes is True
        ...
        self.samples = [(s[0], int(s[1])) for s in self.samples if "samples" not in s[0]] # List of (image path, class_index) tuples 路径从根目录开始？
        self.targets = [s[1] for s in self.samples]
...
class Img2ImgDataset(BasicDataset):
    ...
    def __init__(self, root, transform=None, num_imgs=1, cfg=None, group=0):
        ...
        for i, s in enumerate(self.samples):
            filename = s[0].split("/")[-1] # /mnt/petrelfs/yangshuo/MoMA/output/CUB2011/img/085.Horned_Lark/{idx}-{j}.png -> {idx}-{j}.png
        ...
        for k, v in self.sample_groups.items():
            ...
            self.new_samples += [(v[i][0], int(v[i][1])) for i in chosen] # 选出num_imgs个图片
...
def subsample(dataset1, dataset2, multiples=1, attr='classes', seed=0): # extraset, dataset
    """
    ...
    for c in class_counts.keys():
        ...
        if len(idx_filtered) < class_counts[c]*multiples:
            print(f"Could only get {len(idx_filtered)} samples instead of {class_counts[c]*multiples} for class {c}.")
            replace = True
        else:
            print(f"Getting {class_counts[c]*multiples} samples for class {c}.")
        if len(idx_filtered) == 0:
            continue
        # replace is True than 可取相同的样本
        indices.extend(np.random.choice([i for i, t in enumerate(dataset2.targets) if t == c], class_counts[c]*multiples, replace=replace))
