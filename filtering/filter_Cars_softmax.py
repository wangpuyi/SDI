...
if args.test:
    ...
    print(f"project=SEI-filter, group={args.name}, config={flatten_config(args)}")
...
def semantic_filter_SEI(embeddings, labels, negative_text):
    ...
    remove_idxs = []
    idxs = []
    filtered_per_class = {label.item(): [0, 0] for label in set(labels)}
    with torch.no_grad():
        for idx, img_feature in enumerate(embeddings):
            img_feature = img_feature.unsqueeze(0)
            text = [f"a photo of a car."]
            ...
            if prediction == 0:
                ...
                filtered_per_class[labels[idx].item()][0] += 1
            else:
                remove_idxs.append(idx)
                filtered_per_class[labels[idx].item()][0] += 1
                filtered_per_class[labels[idx].item()][1] += 1
                print("=============================================")
                print("removed img: ", clip_dataset.samples[idx][0])
    for label, counts in filtered_per_class.items():
        print(f"Label {label} total: {counts[0]}, removed: {counts[1]}, ratio: {counts[1]/counts[0]}")
    predictions = torch.tensor(predictions).to("cuda")
    return predictions, remove_idxs, idxs

# customized SEI filter with rank
def semantic_filter_rank(embeddings, labels, negative_text=None):
    """Filter out images that are not similar to the text prompt"""
    model, preprocess = clip.load("ViT-L/14", device="cuda", download_root="/grp01/cs_hszhao/cs002u03/ckpt/clip")
    # predictions = []
    remove_idxs = []
    ...
    for category, sims in category_similarity.items():
        ...
        for _, idx in best_half:
            idxs.append(idx)  # 保留最好的half
        for _, idx in worst_half:
            remove_idxs.append(idx)  # 筛去最差的half
...
# semantic_filtered, kept_idxs = semantic_filter_rank(aug_emb, aug_labels)
semantic_filtered, kept_idxs = semantic_filter_SEI(aug_emb, aug_labels, list(args.filter.negative_prompts))
