...
from ip_adapter.ip_adapter import ImageProjModel, SEIImageProjModel
...
def setup_ddp(rank, world_size):
    ...
    os.environ['MASTER_PORT'] = str(17375)
...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, json_file, tokenizer, size=512, i_drop_rate=0.05, image_root_path=""):
        super().__init__()
    ...
    def __getitem__(self, idx):
        ...
        if "word" in item:
            ...
            raise NotImplementedError("word not exists in the json data")
        label = item["label"]
        ...
        prompt = f"a high-quality photo of a {word}."
...
def parse_args():
    ...
    parser.add_argument("--scale", type=int, default=1, help="For distributed training: local_rank")
...
def main(rank, world_size):
    ...
    wandb_name = "bs{}-lr_{}-initScale{}-phase2-{}".format(
        args.train_batch_size*world_size, args.learning_rate, args.scale, current_time)
    wandb.init(project="train_proj", name=wandb_name, config=args)
    ...
    anchor = torch.from_numpy(np.load('IN100.npy')).to(device)
    ...
    for epoch in range(0, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            ## normalize ##
            feat_norm = F.normalize(projected_image_embeds, p=2, dim=1)
            anchor_normalized = F.normalize(anchor, p=2, dim=1)

            # print(f"shape of projected_image_embeds: {projected_image_embeds.shape}") # torch.Size([32, 768])
            # print(f"shape of anchor: {anchor.shape}") # torch.Size([100, 768])
            # print(f"len(projected_image_embeds): {len(projected_image_embeds)} len(anchor): {len(anchor)}")

            ## cosine similarity ##
            x = F.cosine_similarity(anchor_normalized.unsqueeze(0).repeat(len(projected_image_embeds), 1, 1),
                        feat_norm.unsqueeze(1).repeat(1, len(anchor), 1), dim=2) #(batch_size, num_categories)
            target = torch.tensor(batch["labels"]).to(device)
            # loss_2 = F.cross_entropy(x, target) # average loss

            ## MSE loss ##
            loss_2 = F.mse_loss(projected_image_embeds.float(), anchor[target].float()) # Shape: scalar (average loss over the batch)

            print(f"loss1: {loss_1} loss2: {loss_2}")

            predicted_labels = torch.argmax(x, dim=1)
            num_samples = len(target)
            correctly_classified = (predicted_labels == target).sum().item()
            print(f"batch: {num_samples}")
            print(f"correctly classified: {correctly_classified}")
            scale = args.scale
            loss = loss_1 + scale * loss_2
            ...
            wandb.log({"loss1": loss_1.item()})
            wandb.log({"loss2": loss_2.item()})
            wandb.log({"acc": float(correctly_classified)/num_samples})

            print("Epoch {}, step: {}, data_time: {}, time: {}, loss: {}".format(
                epoch, step, load_data_time, time.perf_counter()-begin, loss
            ))
            # wandb.log("Epoch {}, step: {}, data_time: {}, time: {}, loss: {}".format(
            #     epoch, step, load_data_time, time.perf_counter()-begin, loss
            # ))
            
            global_step += 1
            
            if global_step % args.save_steps == 0: #/1fc-bs64-lr_0.0001
                save_path = os.path.join(args.output_dir+"/bs{}-lr_{}-initScale{}-{}".format(args.train_batch_size*world_size, 
                                                                         args.learning_rate, scale, current_time),
