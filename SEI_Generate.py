...
def load_file(category, args):
    return os.listdir(os.path.join(args.data_root, category))
...
def main(gpu_id, args):
    ...
    for category in sorted(categories)[begin:end]:
        ...
        if  os.path.exists(f"{args.save_root}/{category}") and len(os.listdir(f"{args.save_root}/{category}")) == int(args.expanded_number_per_sample*len(image_paths)):
            ...
            samples = sample_func(args)(data, num_samples=int(args.expanded_number_per_sample * len(image_paths)))
            ...
            for idx in range(len(samples)):
                ...
                img = pipe(prompt_embeds=text_embeds, negative_prompt_embeds=neg_text_embeds, 
                        seed=idx, guidance_scale=args.guidance_scale).images[0]
...
if __name__ == "__main__":
    ...
    # parse.add_argument('--num_gpus', type=int, default=1)
    parse.add_argument('--guidance_scale', type=float, default=7.5)
    ...
    parse.add_argument('--expanded_number_per_sample', type=float, required=True)
