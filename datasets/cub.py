...
class Cub2011(torchvision.datasets.ImageFolder):
    def __init__(self, root, split='train', transform=None, group=0, cfg=None):
        ...
        # self.class_names = self.classes # 001.Black_footed_Albatross
        self.class_names = [c.split('.')[-1].replace('_', ' ') for c in self.classes] # Black footed Albatross
        self.class_map = None
...
#         print(f'root from user: {self.root}')
...
#             lines = f.readlines()
#         self.classes = [l.replace('\n', '').split('.')[-1].replace('_', ' ') for l in lines] #Black footed Albatross
#         self._load_metadata()

#         # if self.split != 'val':
#         #     new_df = []
#         #     for c in self.data.target.unique():
#         #         if subset: # default False
...
#         # self.seg = [os.path.join(self.root, 'CUB_200_2011/segmentations', f.replace('.jpg', '.png')) for f in self.data.filepath]
#         self.targets = [s[1] for s in self.samples]
#         self.groups = [0] * len(self.samples)
#         self.class_weights = get_counts(self.targets)
#         self.class_names = self.data.species.unique() # 001.Black_footed_Albatross

#     def _load_metadata(self):
#         images = pd.read_csv(os.path.join(self.root, 'CUB_200_2011', 'images.txt'), sep=' ',
#                              names=['img_id', 'filepath'])
#         image_class_labels = pd.read_csv(os.path.join(self.root, 'CUB_200_2011', 'image_class_labels.txt'),
#                                          sep=' ', names=['img_id', 'target']) # img_id to class_id; target starts from 1
#         train_test_split = pd.read_csv(os.path.join(self.root, 'CUB_200_2011', 'train_test_split.txt'),
#                                        sep=' ', names=['img_id', 'is_training_img']) # 1 belongs to train set while 0 is test set

#         data = images.merge(image_class_labels, on='img_id')
#         self.data = data.merge(train_test_split, on='img_id')
#         self.data['species'] = self.data['filepath'].apply(lambda x: x.split('/')[0]) # 001.Black_footed_Albatross
